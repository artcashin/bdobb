...
const urls = [env.VITE_OPENBB_API_URL].filter(Boolean);
  ...
  if (strict) {
    console.error(
      "[capabilities] refusing to build: no endpoints configured.\n" +
        ...
        "  Set VITE_OPENBB_API_URL (a repository variable in CI, .env.local\n" +
        "  locally) so the HTTP capability is scoped to the host this build\n" +
        "  actually talks to."
    );
    process.exit(1);
  }
...
console.log(`[capabilities] wrote ${allow.length} HTTP allow rules to default.json`);
